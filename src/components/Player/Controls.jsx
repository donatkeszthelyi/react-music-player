...
const Controls = ({
  ...
  return (
    <>
      {isSmallScreen ? (
        <Box id="controls-container-mobile">
          <Box
            display="flex"
            flexDirection="row"
            alignContent="center"
          ...
          >
            ...
            <Button
              onClick={onSkipNext}
              disabled={isSongsEmpty}
              id="skip-next-button-mobile"
            >
              <SkipNext fontSize="large" />
            </Button>

            {/* Volume Slider */}
            <Box id="volume-slider-container-mobile" alignContent="center">
              <Slider
                value={volume}
                onChange={(event, newValue) => setVolume(newValue / 100)}
                aria-labelledby="volume-slider-mobile"
                valueLabelDisplay="auto"
                valueLabelFormat={(value) => `${Math.floor(value)}%`}
                id="volume-slider-mobile"
              />
            </Box>
          </Box>
          {/* Time Tracker */}
          <Box id="time-tracker-container-mobile">
            <Slider
              value={currentTime}
